...
int bytes = 0, swap, file_count = 0, dir_count = 0, sym_count = 0, dev_count =
		0, fifo_count = 0;
...
int lookup_type[] = { 0, S_IFDIR, S_IFREG, S_IFLNK, S_IFBLK, S_IFCHR, S_IFIFO,
		S_IFSOCK, S_IFDIR, S_IFREG, S_IFLNK, S_IFBLK, S_IFCHR, S_IFIFO,
		S_IFSOCK };

struct test table[] = { { S_IFMT, S_IFSOCK, 0, 's' },
		{ S_IFMT, S_IFLNK, 0, 'l' }, { S_IFMT, S_IFBLK, 0, 'b' }, { S_IFMT,
				S_IFDIR, 0, 'd' }, { S_IFMT, S_IFCHR, 0, 'c' }, { S_IFMT,
				S_IFIFO, 0, 'p' }, { S_IRUSR, S_IRUSR, 1, 'r' }, { S_IWUSR,
				S_IWUSR, 2, 'w' }, { S_IRGRP, S_IRGRP, 4, 'r' }, { S_IWGRP,
				S_IWGRP, 5, 'w' }, { S_IROTH, S_IROTH, 7, 'r' }, { S_IWOTH,
				S_IWOTH, 8, 'w' }, { S_IXUSR | S_ISUID, S_IXUSR | S_ISUID, 3,
				's' }, { S_IXUSR | S_ISUID, S_ISUID, 3, 'S' }, { S_IXUSR
				| S_ISUID, S_IXUSR, 3, 'x' }, { S_IXGRP | S_ISGID, S_IXGRP
				| S_ISGID, 6, 's' }, { S_IXGRP | S_ISGID, S_ISGID, 6, 'S' }, {
				S_IXGRP | S_ISGID, S_IXGRP, 6, 'x' }, { S_IXOTH | S_ISVTX,
				S_IXOTH | S_ISVTX, 9, 't' }, { S_IXOTH | S_ISVTX, S_ISVTX, 9,
				'T' }, { S_IXOTH | S_ISVTX, S_IXOTH, 9, 'x' }, { 0, 0, 0, 0 } };
...
int lookup_entry(struct hash_table_entry *hash_table[], long long start) {
	...
	for (hash_table_entry = hash_table[hash]; hash_table_entry; hash_table_entry
...
int read_block(int fd, long long start, long long *next, void *block) {
	...
	if (SQUASHFS_COMPRESSED(c_byte)) {
		...
		res = comp->uncompress(block, buffer, c_byte, SQUASHFS_METADATA_SIZE,
				&error);
...
int set_attributes(char *pathname, int mode, uid_t uid, gid_t guid,
		time_t time, unsigned int xattr, unsigned int set_mode) {
...
int create_inode(char *pathname, struct inode *i) {
	...
	case SQUASHFS_LCHRDEV_TYPE: {
		...
		if (root_process) {
			...
			set_attributes(pathname, i->mode, i->uid, i->gid, i->time,
					i->xattr, TRUE);
			dev_count++;
		}
else			ERROR("create_inode: could not create %s "
	...
	if (i == paths->names) {
		...
		if (use_regex) {
			...
			if (paths->name[i].preg == NULL)
				EXIT_UNSQUASH("Out of memory in add_path\n");
			error = regcomp(paths->name[i].preg, targname, REG_EXTENDED
					|REG_NOSUB);
			if (error) {
				char str[1024];
...
struct pathnames *add_subdir(struct pathnames *paths, struct pathname *path) {
	if (paths->count % PATHS_ALLOC_SIZE == 0) {
		paths = realloc(paths, sizeof(struct pathnames *) + (paths->count
				+ PATHS_ALLOC_SIZE) * sizeof(struct pathname *));
		if (paths == NULL)
	...
	*new = init_subdir();
	...
	for (n = 0; n < paths->count; n++) {
		...
		for (i = 0; i < path->names; i++) {
			int match = use_regex ? regexec(path->name[i].preg, name,
					(size_t) 0, NULL, 0) == 0 : fnmatch(path->name[i].name,
...
void dir_scan(char *parent_name, unsigned int start_block, unsigned int offset,
	...
	if (!lsonly && mkdir(parent_name, (mode_t) dir->mode) == -1 && (!force
			|| errno != EEXIST))
	...
	while (squashfs_readdir(dir, &name, &start_block, &offset, &type)) {
		...
		if (type == SQUASHFS_DIR_TYPE)
			...
			if (i->type == SQUASHFS_SYMLINK_TYPE || i->type
	...
	if (!lsonly)
		set_attributes(parent_name, dir->mode, dir->uid, dir->guid, dir->mtime,
				dir->xattr, force);
...
#else
	printf("Found a valid %sSQUASHFS %d:%d superblock on %s.\n", sBlk.s.s_major
			== 4 ? "" : swap ? "big endian " : "little endian ",
			sBlk.s.s_major, sBlk.s.s_minor, source);
#endif
	printf("Creation or last append time %s", mkfs_str ? mkfs_str
			: "failed to get time\n");
	printf("Filesystem size %.2f Kbytes (%.2f Mbytes)\n", sBlk.s.bytes_used
			/ 1024.0, sBlk.s.bytes_used / (1024.0 * 1024.0));
	if (sBlk.s.s_major == 4)
...
int read_super(char *source) {
	 ...
	 */
	read_fs_bytes(fd, SQUASHFS_START, sizeof(squashfs_super_block), &sBlk_4);
...
void *reader(void *arg) {
	while (1) {
		...
		int res = read_fs_bytes(fd, entry->block,
				SQUASHFS_COMPRESSED_SIZE_BLOCK(entry->size), entry->data);
...
void *writer(void *arg) {
	...
	while (1) {
		...
		for (i = 0; i < file->blocks; i++, cur_blocks++) {
			...
			error = write_block(file_fd, block->buffer->data + block->offset,
		...
		close(file_fd);
		if (failed == FALSE)
			set_attributes(file->pathname, file->mode, file->uid, file->gid,
					file->time, file->xattr, force);
		else {
...
void *deflator(void *arg) {
	...
	while (1) {
		...
		res
				= comp->uncompress(tmp, entry->data,
...
int _main(int argc, char *argv[]) {
	...
	for (i = 1; i < argc; i++) {
		...
		if (strcmp(argv[i], "-version") == 0 || strcmp(argv[i], "-v") == 0) {
			...
			lsonly = TRUE;
		else if (strcmp(argv[i], "-no-progress") == 0 || strcmp(argv[i], "-n")
				== 0)
			progress = FALSE;
		else if (strcmp(argv[i], "-no-xattrs") == 0 || strcmp(argv[i], "-no")
				== 0)
			no_xattrs = TRUE;
			...
			dest = argv[i];
		} else if (strcmp(argv[i], "-processors") == 0 || strcmp(argv[i], "-p")
				== 0) {
			if ((++i == argc) || (processors = strtol(argv[i], &b, 10), *b
					!= '\0')) {
			...
			if (processors < 1) {
				...
				|| strcmp(argv[i], "-da") == 0) {
			if ((++i == argc)
					|| (data_buffer_size = strtol(argv[i], &b, 10), *b != '\0')) {
			...
			if (data_buffer_size < 1) {
				...
				|| strcmp(argv[i], "-fr") == 0) {
			if ((++i == argc) || (fragment_buffer_size
			...
			lsonly = TRUE;
			short_ls = FALSE;
		} else if (strcmp(argv[i], "-linfo") == 0 || strcmp(argv[i], "-li")
				== 0) {
			info = TRUE;
			short_ls = FALSE;
