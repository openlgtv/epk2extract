...
static void dump_pseudo(struct pseudo *pseudo, char *string) {
	...
	for (i = 0; i < pseudo->names; i++) {
		...
		if (entry->pseudo == NULL)
			ERROR("%s %c %o %d %d %d %d\n", path, entry->dev->type, entry->dev->mode, entry->dev->uid, entry->dev->gid, entry->dev->major, entry->dev->minor);
		else
		...
		if (pseudo->name[i].pseudo == NULL) {
			...
			if (target[0] != '\0') {
				...
				if (pseudo->name[i].dev->type == 'd')
					...
					ERROR("%s already exists as a non " "directory.  Ignoring %s!\n", targname, alltarget);
			} else if (memcmp(pseudo_dev, pseudo->name[i].dev, sizeof(struct pseudo_dev)) != 0)
				ERROR("%s already exists as a different pseudo " "definition.  Ignoring!\n", alltarget);
			else
				ERROR("%s already exists as an identical " "pseudo definition!\n", alltarget);
		} else {
			/* sub-directory exists which means this can only be a
			 * 'd' type pseudo file */
			if (target[0] == '\0') {
				if (pseudo->name[i].dev == NULL && pseudo_dev->type == 'd') {
					pseudo->name[i].pathname = strdup(alltarget);
					pseudo->name[i].dev = pseudo_dev;
				} else
					ERROR("%s already exists as a " "directory.  Ignoring %s!\n", targname, alltarget);
			} else
				/* recurse adding child components */
...
int read_pseudo_def(struct pseudo **pseudo, char *def) {
	...
	n = sscanf(def, "%s %c %o %s %s %n", filename, &type, &mode, suid, sgid, &bytes);

	if (n < 5) {
		ERROR("Not enough or invalid arguments in pseudo file " "definition\n");
		goto error;
	}
	...
	case 'f':
		if (def[bytes] == '\0') {
			ERROR("Not enough arguments in pseudo file " "definition\n");
			goto error;
		}
		break;
	case 'd':
	...
	if (type == 'f') {
		...
		if (res == -1) {
			ERROR("Failed to execute dynamic pseudo file definition" " \"%s\"\n", def);
			return FALSE;
		}
		add_pseudo_file(dev);
	}
