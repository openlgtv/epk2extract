...
int read_block(int fd, long long start, long long *next, void *block) {
	...
	if (SQUASHFS_COMPRESSED(c_byte)) {
		...
		res = compressor_uncompress(comp, block, buffer, c_byte, SQUASHFS_METADATA_SIZE, &error);
		if (res == -1) {
...
int scan_inode_table(int fd, long long start, long long end, long long root_inode_start, int root_inode_offset, struct squashfs_super_block *sBlk, union squashfs_inode_header *dir_inode, unsigned char **inode_table, unsigned int *root_inode_block, unsigned int *root_inode_size, long long *uncompressed_file, unsigned int *uncompressed_directory, int *file_count, int *sym_count, int *dev_count, int *dir_count, int *fifo_count, int *sock_count, unsigned int *id_table) {
	...
	*root_inode_size = bytes - (*root_inode_block + root_inode_offset);
	...
	if (dir_inode->base.inode_type == SQUASHFS_DIR_TYPE) {
		...
		SQUASHFS_SWAP_LDIR_INODE_HEADER(&dir_inode->ldir, *inode_table + bytes);
	...
	for (cur_ptr = *inode_table; cur_ptr < *inode_table + bytes; files++) {
		...
		case SQUASHFS_FILE_TYPE:{
				...
				int blocks = inode.fragment == SQUASHFS_INVALID_FRAG ? (inode.file_size + sBlk->block_size - 1) >> sBlk->block_log : inode.file_size >> sBlk->block_log;
		...
		case SQUASHFS_LDIR_TYPE:{
				...
				for (i = 0; i < dir_inode.i_count; i++) {
					...
					cur_ptr += sizeof(struct squashfs_dir_index) + index.size + 1;
				}
				break;
			}
...
int read_fragment_table(int fd, struct squashfs_super_block *sBlk, struct squashfs_fragment_entry **fragment_table) {
	...
	*fragment_table = malloc(sBlk->fragments * sizeof(struct squashfs_fragment_entry));
	...
	for (i = 0; i < indexes; i++) {
		int length = read_block(fd, fragment_table_index[i], NULL,
								((unsigned char *)*fragment_table) + (i * SQUASHFS_METADATA_SIZE));
