...
extern struct compressor *read_super(int fd, struct squashfs_super_block *sBlk, char *source);
extern long long read_filesystem(char *root_name, int fd, struct squashfs_super_block *sBlk, char **cinode_table, char **data_cache, char **cdirectory_table, char **directory_data_cache, unsigned int *last_directory_block, unsigned int *inode_dir_offset, unsigned int *inode_dir_file_size, unsigned int *root_inode_size, unsigned int *inode_dir_start_block, int *file_count, int *sym_count, int *dev_count, int *dir_count, int *fifo_count, int *sock_count, long long *uncompressed_file, unsigned int *uncompressed_inode, unsigned int *uncompressed_directory, unsigned int *inode_dir_inode_number, unsigned int *inode_dir_parent_inode, void (push_directory_entry) (char *, squashfs_inode, int, int), struct squashfs_fragment_entry **fragment_table, squashfs_inode ** inode_lookup_table);
...
extern int generate_file_priorities(struct dir_info *dir, int priority, struct stat *buf);
...
int create_inode(squashfs_inode * i_no, struct dir_info *dir_info, struct dir_ent *dir_ent, int type, long long byte_size, long long start_block, unsigned int offset, unsigned int *block_list, struct fragment *fragment, struct directory *dir_in, long long sparse) {
	...
	int inode_number = type == SQUASHFS_DIR_TYPE ? dir_ent->inode->inode_number : dir_ent->inode->inode_number + dir_inode_no;
...
INSERT_LIST(fragment, struct frag_locked) REMOVE_LIST(fragment, struct frag_locked)
...
void reader_read_process(struct dir_ent *dir_ent) {
	...
	prev_buffer->fragment = !no_fragments && (count == 2 || always_use_fragments) && (byte < block_size);
	queue_put(from_reader, prev_buffer);
...
void reader_read_file(struct dir_ent *dir_ent) {
 ...
 again:
	...
	frag_block = !no_fragments && (always_use_fragments || (read_size < block_size)) ? read_size >> block_log : -1;
...
void *writer(void *arg) {
	...
	while (1) {
		...
		if (file_buffer == NULL) {
			queue_put(from_writer, write_error ? &write_error : NULL);
			continue;
		}
...
void write_file(squashfs_inode * inode, struct dir_ent *dir_ent, int *duplicate_file) {
 ...
 again:
	...
	if (read_size == -1)
		status = write_file_process(inode, dir_ent, read_buffer, duplicate_file);
	else if (read_size == 0) {
...
inline void add_dir_entry(char *name, char *pathname, struct dir_info *sub_dir, struct inode_info *inode_info, struct dir_info *dir) {
	if ((dir->count % DIR_ENTRIES) == 0) {
		dir->list = realloc(dir->list, (dir->count + DIR_ENTRIES) * sizeof(struct dir_ent *));
		if (dir->list == NULL)
	...
	if (dir->count < old_root_entries) {
		...
		for (i = 0; i < old_root_entries; i++) {
			if (old_root_entry[i].inode.type == SQUASHFS_DIR_TYPE)
				dir->directory_count++;
			add_dir_entry(old_root_entry[i].name, "", NULL, &old_root_entry[i].inode, dir);
		}
	}
...
struct dir_ent *scan3_readdir(struct directory *dir, struct dir_info *dir_info) {
	...
	while ((current_count = dir_info->current_count++) < dir_info->count)
		if (dir_info->list[current_count]->inode->root_entry)
			add_dir(dir_info->list[current_count]->inode->inode, dir_info->list[current_count]->inode->inode_number, dir_info->list[current_count]->name, dir_info->list[current_count]->inode->type, dir);
		else
			return dir_info->list[current_count];
	return NULL;
}
...
struct dir_info *dir_scan1(char *pathname, struct pathnames *paths, int (_readdir) (char *, char *, struct dir_info *)) {
	...
	while (_readdir(filename, dir_name, dir) != FALSE) {
		...
		add_dir_entry(dir_name, filename, sub_dir, lookup_inode(&buf), dir);
	}
...
struct dir_info *dir_scan2(struct dir_info *dir, struct pseudo *pseudo) {
	...
	while ((pseudo_ent = pseudo_readdir(pseudo)) != NULL) {
		...
		if (pseudo_ent->dev->type == 'm') {
			struct stat *buf;
			if (dir_ent == NULL) {
				ERROR("Pseudo modify file \"%s\" does not exist " "in source filesystem.  Ignoring.\n", pseudo_ent->pathname);
				continue;
			}
			if (dir_ent->inode->root_entry) {
				ERROR("Pseudo modify file \"%s\" is a pre-existing" " file in the filesystem being appended" "  to.  It cannot be modified. " "Ignoring.\n", pseudo_ent->pathname);
				continue;
			}
		...
		if (dir_ent) {
			if (dir_ent->inode->root_entry)
				ERROR("Pseudo file \"%s\" is a pre-existing" " file in the filesystem being appended" "  to.  Ignoring.\n", pseudo_ent->pathname);
			else
				ERROR("Pseudo file \"%s\" exists in source " "filesystem \"%s\".\nIgnoring, " "exclude it (-e/-ef) to override.\n", pseudo_ent->pathname, dir_ent->pathname);
			continue;
		}
		...
		if (pseudo_ent->dev->type == 'd') {
			...
			if (sub_dir == NULL) {
				ERROR("Could not create pseudo directory \"%s\"" ", skipping...\n", pseudo_ent->pathname);
				continue;
			}
			dir->directory_count++;
		} else
		...
		if (pseudo_ent->dev->type == 'f') {
			...
			struct inode_info *inode;
			if (res == -1) {
				ERROR("Stat on pseudo file \"%s\" failed, " "skipping...", pseudo_ent->pathname);
				continue;
			}
			buf.st_size = buf2.st_size;
			inode = lookup_inode(&buf);
			inode->pseudo_file = PSEUDO_FILE_OTHER;
			add_dir_entry(pseudo_ent->name, pseudo_ent->dev->filename, sub_dir, inode, dir);
#else
			struct inode_info *inode = lookup_inode(&buf);
			inode->pseudo_id = pseudo_ent->dev->pseudo_id;
			inode->pseudo_file = PSEUDO_FILE_PROCESS;
			add_dir_entry(pseudo_ent->name, pseudo_ent->pathname, sub_dir, inode, dir);
#endif
		} else {
			struct inode_info *inode = lookup_inode(&buf);
			inode->pseudo_file = PSEUDO_FILE_OTHER;
...
void dir_scan3(squashfs_inode * inode, struct dir_info *dir_info) {
	...
	while ((dir_ent = scan3_readdir(&dir, dir_info)) != NULL) {
		...
		unsigned int inode_number = ((buf->st_mode & S_IFMT) == S_IFDIR)
			? dir_ent->inode->inode_number : dir_ent->inode->inode_number + dir_inode_no;
		...
		if (dir_ent->inode->inode == SQUASHFS_INVALID_BLK) {
			...
			case S_IFREG:
				...
				INFO("file %s, uncompressed size %lld " "bytes %s\n", filename, (long long)buf->st_size, duplicate_file ? "DUPLICATE" : "");
				break;
...
void initialise_threads(int readb_mbytes, int writeb_mbytes, int fragmentb_mbytes) {
	int i;
...
long long write_inode_lookup_table() {
	...
	for (i = 0; i < INODE_HASH_SIZE; i++) {
		...
		for (inode = inode_info[i]; inode; inode = inode->next) {
			...
			inode_number = inode->type == SQUASHFS_DIR_TYPE ? inode->inode_number : inode->inode_number + dir_inode_no;
...
int main(int argc, char *argv[]) {
		...
		if (strcmp(argv[i], "-comp") == 0) {
			...
			if (args < 0) {
				if (args == -1) {
					...
					ERROR("%s: Did you forget to specify" " -comp, or specify it after" " the compressor specific" " option?\n", argv[0]);
				}
				exit(1);
			}
			i += args;
 ...
 printOptions:
			...
			ERROR("-fragment-queue <size>\tSet fragment queue to " "<size> Mbytes.  Default %d Mbytes\n", FRAGMENT_BUFFER_DEFAULT);
	...
	for (i = source + 2; i < argc; i++)
		if (strcmp(argv[i], "-ef") == 0) {
			...
			break;
		else if (strcmp(argv[i], "-root-becomes") == 0 || strcmp(argv[i], "-sort") == 0 || strcmp(argv[i], "-pf") == 0 || strcmp(argv[i], "-comp") == 0)
			i++;
	...
	for (i = source + 2; i < argc; i++)
		if (strcmp(argv[i], "-sort") == 0) {
			...
			break;
		else if (strcmp(argv[i], "-root-becomes") == 0 || strcmp(argv[i], "-ef") == 0 || strcmp(argv[i], "-pf") == 0 || strcmp(argv[i], "-comp") == 0)
			i++;
