...
int API_SWU_VerifyImage(void *signature, void* data, size_t imageSize) { 
	...
	EVP_DigestUpdate(&md_ctx, data, imageSize);
	EVP_DigestFinal(&md_ctx, (unsigned char *)&md_value, &md_len);
	EVP_DigestInit(&md_ctx, EVP_sha1());
	EVP_DigestUpdate(&md_ctx, (unsigned char *)&md_value, md_len);
	int result = EVP_VerifyFinal(&md_ctx, signature, SIGNATURE_SIZE, _gpPubKey);
